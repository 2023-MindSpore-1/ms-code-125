...
def load_processed_data_info(data_name):
    ...
    # get_adj_bool
    edge_index = target_data.get_all_neighbors(node_list=nodes, neighbor_type=0, output_format=OutputFormat.COO).T
    target_data.adj_bool=index2adj_bool(edge_index,target_data.num_nodes)

    # get_global_effect_matrix
    A     = index2dense(target_data.get_all_neighbors(node_list=nodes, neighbor_type=0, output_format=OutputFormat.COO).T, len(target_data.get_all_nodes(0).tolist()))
    eye = ops.Eye()
